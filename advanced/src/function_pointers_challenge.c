...
FuncPointer commandList[4] = {&add, &sub, &mult, &divi};

// performs the given operation on elements of the arrays
int* performOp(int *a, int *b, int size, FuncPointer command);
...
int main( ){
  ...
  while(choice != 5)
  {
	...
	result = performOp(array1,array2,size, commandList[j]);		
